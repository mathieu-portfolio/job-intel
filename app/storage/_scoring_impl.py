    ...
    *,
    ...
    with _connect(db_path) as connection:
        save_offer_scores_batch(
            connection,
            [(offer_id, profile_path, preset_id, evaluation)],
            scored_at=scored_at,
        )
...
def save_offer_scores(
    *,
    ...
    with _connect(db_path) as connection:
        save_offer_scores_batch(
            connection,
            [(offer_id, profile_path, preset_id, evaluation) for preset_id, evaluation in evaluations.items()],
            scored_at=scored_at,
        )
...
def save_offer_scores_batch(
    ...
    rows: list[tuple[int, str, str, RuleEvaluation]],
    *,
    scored_at: str | None = None,
) -> None:
    ...
    connection.executemany(
        ...
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(offer_id, profile_path, preset_id) DO UPDATE SET
        ...
        [
            ...
            for offer_id, profile_path, preset_id, evaluation in rows
        ],
    )
...
def select_screened_offers(
    *,
    ...
    init_db(db_path)
    params: list[Any] = [profile_path, preset_id]
    min_score_clause = ""
...
def list_screened_offers(
    *,
    ...
    if profile_path:
        clauses.insert(0, "offer_scores.profile_path = ?")
        where_params.insert(0, profile_path)
    ...
    with _connect(db_path) as connection:
        rows = connection.execute(
            ...
            LEFT JOIN ai_reviews
             ...
             AND ai_reviews.profile_path = offer_scores.profile_path
            WHERE {where_sql}
            ORDER BY {order_by}
            LIMIT ?;
            """,
